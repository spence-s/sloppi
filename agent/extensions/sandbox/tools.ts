...
export class SandboxTools {
  ...
  register(): void {
    ...
    const read: ReadOperations = {
      ...
      async readFile(path) {
        const result = await sandbox.run`base64 < ${realpathSync(path)} | tr -d '\n'`;
        if (result.exitCode !== 0) {
          throw new Error(result.stderr.trim().length > 0 ? result.stderr.trim() : `Cannot read ${path}`);
        }
    ...
    const edit: EditOperations = {
      ...
      ...write,
      async access(path) {
        const result = await sandbox.run`test -r ${realpathSync(path)} && test -w ${realpathSync(path)}`;
        if (result.exitCode !== 0) {
    ...
    const find: FindOperations = {
      async exists(path) {
        const result = await sandbox.run`test -e ${realpathSync(path)}`;
        return result.exitCode === 0;
      },
      async glob(pattern, path, {ignore, limit}) {
        const name = pattern.includes('/') ? '-path' : '-name';
        const match = name === '-path' ? `*${pattern}` : pattern;
        const result = await sandbox.run`${['find', realpathSync(path), '-type', 'f', ...ignore.flatMap(entry => ['!', '-path', `*${entry}`]), name, match, '-print']}`;
        if (result.exitCode !== 0) {
    ...
    const ls: LsOperations = {
      ...
      async stat(path) {
        const exists = await sandbox.run`test -e ${realpathSync(path)}`;
        if (exists.exitCode !== 0) {
          throw new Error(`Path not found: ${path}`);
        }

        const directory = await sandbox.run`test -d ${realpathSync(path)}`;
        return {isDirectory: () => directory.exitCode === 0};
      },
      async readdir(path) {
        const result = await sandbox.run`ls -1A -- ${realpathSync(path)}`;
        if (result.exitCode !== 0) {
