...
export function mergeSandboxConfig(base: Record<string, unknown>, override: Record<string, unknown>): Record<string, unknown> {
  const merged = {...base};
...
function getProjectConfig(config: Record<string, unknown>, cwd: string): Record<string, unknown> {
...
export function getEffectiveConfig(config: Record<string, unknown>, cwd: string): Record<string, unknown> {
...
export function createConfigStore(
  ...
  const reload = async (): Promise<Record<string, unknown>> => {
    try {
  ...
  const load = async (): Promise<Record<string, unknown>> => hasLoaded ? config : reload();

  const getScope = (scope: ConfigScope): Record<string, unknown> => {
    if (scope === 'global') {
      return config;
    }

    const {projects: savedProjects} = config;
    const projects: Record<string, unknown> = isConfig(savedProjects) ? savedProjects : {};
