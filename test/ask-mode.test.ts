...
void describe('ask-mode', () => {
  ...
  void test('onToolCall allows read when ask mode is enabled', (t: TestContext) => {
    const result = onToolCall(createToolCall('read'), true);
    t.assert.strictEqual(result, undefined);
  });
  ...
  void test('ask on restricts active tools to read and sets enabled LLM context', async (t: TestContext) => {
    const harness = createHarness(['read', 'bash', 'edit', 'write']);

    await harness.getCommand('ask').handler('on', harness.ctx);

    t.assert.deepStrictEqual(harness.getActiveTools(), ['read']);
